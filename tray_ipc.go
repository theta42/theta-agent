...
type TrayStatus struct {
	...
	// Mesh exit selection. The Directory has always computed the allowed set
	// "so the UI and the agent tray render the same set" -- this is the tray
	// half, which did not exist before. CurrentExitSiteID is nil for local
	// breakout (no exit).
	Exits             []TrayExit `json:"exits,omitempty"`
	CurrentExitSiteID *int       `json:"current_exit_site_id,omitempty"`
}

// TrayExit is one site the device may route its internet traffic through.
...
type TrayCommand struct {
	Command string `json:"command"` // "set_auto_vpn", "vpn_connect", "vpn_disconnect", "set_exit"
