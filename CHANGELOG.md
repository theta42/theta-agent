...
### Fixed
- **Every Command Response Was Discarded By The Directory**: `sendResponse` wrote a bare `{"status": …, "message": …}` with no `type`, and the directory drops any frame without a string `type` — silently, since an unparseable frame is not worth a log line per connection. So *no* command answer this agent ever sent was recorded: the fleet view's "last response" was permanently null, `arbitrary_bash` and `fetch_logs` output never reached the UI, and the directory's `case 'response'` handler was unreachable code. All five rich responses (`fetch_logs`, `desktop_control`, `systemd_action`, `arbitrary_bash`, `zpool_scrub`) wrote the same envelope-less shape. Everything now goes out as the `{type: "response", payload}` envelope PROTOCOL.md §3.4 has always specified, and `desktop_control`/`systemd_action` report `status: "error"` when the underlying command failed instead of `"ok"` with an `error` field nobody reads. A new test helper asserts the envelope, so the contract is covered rather than assumed.
- **Re-Enrollment Was A One-Way Door (contract G-2, agent half)**: the directory has required `?prev_token=` — the token the host held before its enrollment was cleared — before a join key may re-enroll a hostname it already knows, since the H1 takeover fix. The agent never implemented its half: `prev_token` appeared nowhere in the codebase and `ClearEnrollment` blanked `auth_token` outright. So `theta-agent reset-enrollment` and the tray's re-enroll both left the host dialling with its join key, colliding with its own hostname, and rejected `4001` with a 5-minute backoff **forever** — recoverable only by deleting the agent row in the directory by hand. `ClearEnrollment` now moves the superseded token to `prev_auth_token`, the dial presents it in an `X-Theta-Prev-Token` header (a header, not the query string, for the same reason the credential itself is one), and `PersistEnrollment` clears it once spent. It is never returned by `Credential()` — it is proof of continuity, not a credential.
- **Tray "Re-Enroll" Could Brick A Host**: `reinit` called `ClearEnrollment` unconditionally, while the CLI's `reset-enrollment` refuses when `agent.yml` holds an `auth_token` but no `join_key` — which is the normal post-enrollment state, since the join key is blanked once the host holds its own token. Pressing it left the daemon looping on "nothing to authenticate with" until an operator edited the file by hand. The tray path now applies the same guard and logs which file to fix.
- **A Pushed IAM Policy Could Lock Root Out Of The Machine**: `applyAccessControl` wrote the directory's group list into `/etc/security/access.conf` followed by `-:ALL:ALL`. Root is in no LDAP group, so a pushed list that did not happen to cover a host's administrators denied every login including the console — and the console is the only way back in. `+:root:ALL` is now written first, whatever the directory sent: a remote policy push must not be able to make a host unrecoverable.
- **Capabilities Reported Two Things That Were Not True**: `shutdown` was reported as unconditionally available while the command is gated by the *reboot* capability, so the directory offered a button that could only ever be refused; and `storage` — the gate on `zpool_scrub` — was not reported at all, leaving no way to know whether a scrub would be accepted.

### Added
- **Telemetry: `uptime_seconds` And `wireguard` State**: the directory's driver has always read both off the telemetry frame and neither was ever sent, so host uptime rendered as null and a `wireguard` resource rendered an empty peer list that meant nothing. `wireguard` carries `{active, ready}` — whether the interface is up, and whether `wg`/`wg-quick` are installed at all, which is what separates "configured and down" from "this host can never bring it up". Both come from state the agent already has; no extra process per tick.

### Changed
- **`services` Is Always On The Wire, Even When Empty**: it was `omitempty`, so "I have no services any more" was indistinguishable from "I am an older agent that does not report services" — and the directory consequently could never prune a service child, making PROTOCOL.md §3.5's promise that a service removed from `agent.yml` loses its resource "on the next reconciliation" false since it was written. An empty list is now explicit, which is what lets the directory prune safely.
- **`install.sh` Fetches The Binary From The Directory First**: `setup.sh` stages every artifact for the suite's pinned agent version into the directory's `/resources/theta-agent/` and serves it with a `SHA256SUMS` — which is already where `theta-agent update` fetches from — but this script went straight to GitHub's `releases/latest` regardless. An air-gapped or egress-filtered host could not install at all despite the bytes sitting on the directory it was being pointed at, and a host that *could* reach GitHub landed on a different build than the directory serves, so its first self-update moved it backwards. It now prefers the directory (verifying against the served manifest) and falls back to GitHub.
- **PROTOCOL.md v1.4.0**: documents `?prev_token=` / `X-Theta-Prev-Token` (§1.1), the `site_name` and `organization_name` config fields (§4.0) which were being sent and written down nowhere, the response envelope and why a typeless frame is dropped (§3.4), the new telemetry fields (§3.2), service-pruning semantics (§3.5), `zpool_scrub` in the signed-command table, a new §4.6 on who actually sends `iam_apply` and which parts of the payload are deliberately *not* derived, and a warning that the signed-command table and the directory's `HIGH_RISK_COMMANDS` drifting apart is silent in both directions.
...
### Added
- **`theta-agent verify`** — check that this host's configuration and key material are usable, without changing anything. Confirms `agent.yml` parses, `server_url` is a URL with a host, a credential is present, the Directory's `public_key` decodes to a 32-byte Ed25519 key, and the WireGuard private key (if generated) is a well-formed 32-byte Curve25519 key with sane permissions. Exits non-zero on anything fatal, so it can gate a script — `install.sh` now runs it after writing configuration. A `public_key` that does not decode used to produce an agent that started, connected, and then rejected every signed command, visible only in the journal on a host the operator had just been told installed successfully.
- **Man page.** `man theta-agent` documents every command, plus the files it owns. Generated by the binary itself (`theta-agent help --man`) from the same registry that backs the interactive help, so the two cannot describe different CLIs. `install.sh` writes it to `/usr/share/man/man8`.
- **`--tray` / `--no-tray`** on `install.sh`, overriding the automatic desktop detection below.

### Fixed
- **`theta-agent <command> help` printed the error message, not documentation.** `theta-agent register help` answered with `[!] usage: theta-agent <register|unregister> <type> <name>` — nothing about what the types mean, what appears in the Directory, or how to undo it. Help is now hierarchical: `help`, `help <command>`, `<command> help`, `--help` and `-h` all reach the command's own documentation, and the top-level list is one line per command with the detail behind `help <command>`.
- **Four hand-maintained command lists had drifted.** `printUsage()`, the two embedded completion scripts, and the two files under `completions/` were separate copies — `config-set` and `discover` were missing from both files under `completions/`, so neither ever completed, and nothing described `run`. All of them are generated from one registry now, with a test holding the checked-in files to it.
- **The installer replaced the agent binary while the old one was still running.** On Linux the running process stays on the previous inode, so the host kept running the old agent until something restarted it — an upgrade that reported success and changed nothing. The service is now stopped first (and refused outright if it will not stop), per-user tray units are stopped so none holds a stale IPC connection, and the service is restarted afterwards only if it was running to begin with — an operator who had deliberately stopped the agent should not find it running again because they installed a newer build.
- **The installer put a desktop tray on every host, headless servers included** — a systray binary, a `graphical-session.target` user unit that can never activate, and a line in the log promising it "will start at the next desktop login". Detection now looks for a live graphical logind session, an enabled display manager, installed session definitions under `/usr/share/{x,wayland}sessions`, or a live X socket; any one is sufficient.

  `systemctl get-default` is deliberately **not** one of the signals, despite looking like the authoritative answer. Measured on two real headless theta-suite servers, both reported `graphical.target` while having no display manager, no session definitions and no X socket — plenty of server images ship that default, and trusting it would have called every one of those hosts a desktop.
- A tray download that failed used to pass in silence, leaving a desktop with no tray and no explanation.

## [v2.12.0] - 2026-08-24

### Fixed
- **Local discovery could break TLS for the entire machine.** When the site's gateway announces itself over mDNS, the agent repoints the directory's hostname at the announced LAN address in the platform hosts file. It never checked that the LAN address actually serves a certificate valid for that name — and where TLS terminates somewhere the LAN address is not behind (a wildcard cert on a border reverse proxy, a tunnel provider, a corporate ingress), it does not. The suite's own OpenResty data plane answers any SNI it has not issued for with its self-signed `sni-support-required-for-valid-ssl` fallback, so the override turned a working public path into a hard TLS failure — and because `/etc/hosts` is system-wide, it broke the browser, `curl` and everything else on the host, not just the agent.

  The override is now verified before it is committed: a full handshake against the discovered address, system trust store, SNI set to the real hostname — the same verification an ordinary client will do a moment later. It is re-checked on every poll and reverted if it stops holding, so an expired or rotated certificate no longer leaves a machine-wide breakage behind. An internal CA in the system trust store passes, which is the case this is meant to allow; a self-signed fallback does not, which is the case that broke. A plain-`http` directory is exempt — there is no certificate to break.

- **A stale hosts block survived every restart.** The managed block outlives the process that wrote it, and the "already resolves to the discovered address, no override needed" shortcut reads it back through the resolver — so the agent compared its own previous answer against itself, concluded there was nothing to do, and left the block in place permanently. A block written by an older version therefore could not be cleared by upgrading. Discovery now clears any block it finds at startup and re-derives it from real DNS.

- **An agent sitting on its own site's LAN reported "away from home".** Reachability-based detection (v2.11.0) only ever probed an endpoint it could tell was home-only, and it decided that from the *literal*: a private address, or a LAN endpoint the Directory had pushed. Neither is present in a normal deployment — the directory is named by a public FQDN, and a site with no `dnsHost` recorded sends no LAN endpoint — so no endpoint was ever probed, `site_public_ip` was unset, and detection fell straight through to "assume away" once a minute, forever. Auto-VPN dutifully brought a tunnel up on the home LAN.

  Local discovery had the answer the whole time and was throwing it away. mDNS is link-local by construction — multicast does not cross routers or VLANs — so "the site answered on this link" *is* "I am on the site's LAN", needing no address comparison, no probe and no assumption about how the site is addressed. That sighting is now the primary signal, recorded on every discovery poll and trusted only while fresh, so a disabled or wedged discovery loop falls through to the weaker signals instead of answering from a stale reading. Endpoint probing also now resolves names rather than only inspecting literals, so split-horizon DNS — or discovery's own hosts override — counts as the home signal it plainly is.

- **Mesh enrolment ran once, with the wrong credential, and never again.** A self-enrolling host dials the WebSocket with its **join key** and is issued its real auth token over that same connection moments later. Enrolment fired on connect with a config snapshot taken before the dial, and `/api/v1/agent/mesh/enroll` authenticates agents by their issued token and rejects a join key outright — so the one and only attempt returned 401 roughly a second before the credential that would have worked was written to `agent.yml`. With the connection then staying up for days, the device never enrolled at all: no mesh row, nothing in jump-host's device list, and no config to push.

  Enrolment now reads the live config on each attempt rather than a snapshot (retrying a captured join key would just resend the credential that cannot work), waits for the auth token instead of spending its attempt on the join key, and retries a rejection. A `404`/`501` still stops immediately — a Directory without the endpoint will not grow one mid-connection.
